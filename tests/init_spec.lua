...
describe("miniobsidian init", function()
  ...
  it("generates valid Unicode slugs without emoji fragments or punctuation", function()
    local slug = require("miniobsidian").default_config().note_id_func
    assert.equals("hello", slug("hello 😀"))
    assert.equals("中文测试", slug("中文，测试！"))
    assert.equals("我的笔记-2026", slug("我的笔记 2026"))
    assert.equals("日本語-かな-カナ-한글", slug("日本語 かな カナ 한글"))
    assert.equals("a-b", slug(" A & B! "))
    assert.equals("", slug("😀"))
  end)
end)
